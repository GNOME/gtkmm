...
install: all
	...
	@for %h in ($(GTKMM_INT_GENERATED_HEADERS_P)) do @if exist ..\gtk\gtkmm\private\%h copy ..\gtk\gtkmm\private\%h "$(PREFIX)\include\gtkmm-$(GTKMM_MAJOR_VERSION).$(GTKMM_MINOR_VERSION)\gtkmm\private\%h"
	@for %h in ($(GTKMM_INT_GENERATED_HEADERS_P)) do @if exist vs$(PDBVER)\$(CFG)\$(PLAT)\gtkmm\private\%h copy vs$(PDBVER)\$(CFG)\$(PLAT)\gtkmm\private\%h "$(PREFIX)\include\gtkmm-$(GTKMM_MAJOR_VERSION).$(GTKMM_MINOR_VERSION)\gtkmm\private\%h"
